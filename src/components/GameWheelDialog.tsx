...
export function GameWheelDialog() {
  ...
  const { gameWheelBackground } = useBackgroundSettings();
  ...
  // Function to get the background gradient based on the selected background type
  const getBackgroundStyle = (background: string) => {
    ...
    return gradients[background as keyof typeof gradients] || gradients.staticPink;
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent 
        ...
        style={{ background: getBackgroundStyle(gameWheelBackground) }}
      >
        <div className="flex flex-col h-full max-h-[85vh]">
          <DialogHeader className="p-3 pb-2 border-b border-rose-100/50 flex-shrink-0 rounded-t-2xl">
            {showReviewDetails ? (
